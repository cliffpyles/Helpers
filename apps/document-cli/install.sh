...
if [ ! -f "$HOME_DIR/.document/config.yaml" ]; then
  mkdir "$HOME_DIR/.document"
  touch "$HOME_DIR/.document/config.yaml"
fi
...
ln -s "$SCRIPT_DIR/document_cli.py" "$HOME_DIR/bin/document"
...
echo "Type 'document' to use the document-cli app."